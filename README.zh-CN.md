...
当前仓库的实际实现是本地 OCR 加确定性规则和版面关系，不依赖生成式大模型。需要增强自然语言和复杂上下文判断时，可以在 OCR 后接入 4B 参数以下、可在本机运行的小语言模型：OCR 提供文字和坐标，小模型只判断哪些内容属于隐私，再把判断结果映射回原图遮挡。身份证、电话和医疗编号等明确格式仍由校验规则兜底。
...
| 姓名、地址、单位和复杂上下文 | 4B 参数以下的本地小语言模型 | 结合 OCR 上下文判断哪些内容属于隐私，不需要单独部署 BERT 或 RoBERTa 实体识别模型 |
...
一个实用的本地方案可以只保留三层：轻量 OCR 读取文字和位置，4B 参数以下的小语言模型判断隐私语义，确定性规则与人工复核负责安全兜底。模型越大不代表越安全，真实领域样本、处理后复检和人工审核共同决定最终可靠性。
